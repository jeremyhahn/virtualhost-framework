...
qx.Class.define( "packages.core.ui.window.NewPackage", {

  extend : p2phost.Main,

  properties : {

  	    Parent        : { init : p2phost.Main.getInstance() },
  	    NewPackageWin : { init : new qx.ui.window.Window( "Install New Packages", "icon/16/mimetypes/package-x-generic.png" ) },
  ...
  construct : function( arrArgs ) {
      ...
      this.setParentPkgLv( arrArgs[0] );
      this.setParentPkgLd( arrArgs[1] );
  ...
  */
  ...
  members : {
			  	  ...
			  	  req.addEventListener( "completed", function( e ) {
		              ...
		              try {
                            ...
                            for( var j=0; j<objSitePackages.length; j++ ) {
                                 ...
                                 if( arrPackageNames.indexOf( objSitePackages[j].namespace ) != -1 )
                                     continue;
    ...
    main : function() {
          ...
          var at1 = new qx.ui.basic.Atom( "Highlight the package(s) you wish to install and click the<br>Install button to proceed with the installation.", "icon/22/mimetypes/package-x-generic.png" );
	      at1.setLocation( 4, 4 );
          ...
          var installBtn = new qx.ui.form.Button( "Install", "icon/16/apps/internet-download-manager.png" );
              installBtn.set({ bottom : 15, right : 10, enabled : false });
              installBtn.addEventListener( "execute", function( e ) { this.installPackage( pkgLv, pkgLd, installBtn ) }, this );
          ...
          for( var i=0; i<items.length; i++ ) {
               ...
               for( var j=0; j<selectedItems.length; j++ ) {
                   ...
                   if( items[i].namespace.text == selectedItems[j].namespace.text ) {
                       ...
                       // Let the user know which package is being installed
                       this.getNewPackageWin().setStatus( "Installing package " + selectedItems[j].displayName.text + "..." );

                       // Disable the install button while installation is taking place
                       ...
                       for( var k=0; k<this.getObjPackages().length; k++ ) {
                            ...
                            if( this.getObjPackages()[k].namespace == items[i].namespace.text ) {
                               ...
                               // Invoke the RPC method on the server which installs the package
				               var callback = qx.lang.Function.bind( this.__installPackageHandler, this );
                               this.getParent()._callAsyncRpc( callback, "core.rpc.ui.Packages", "installPackage",
     ...
     __installPackageHandler : function( result, ex, id ) {
        ...
        if( result ) {
            ...
            // Populate the packages listview ith the new package properties
            this.getParentPkgLd().push({ Name      : { text : this.getPkgName() },
               	                         Namespace : { text : this.getPkgNamespace() },
		                                 Status    : { text : "active" },
		                                 Version   : { text : this.getPkgVersion() }
  					                   });
   			this.getParentPkgLv().updateSort();
            this.getParentPkgLv().update();

            // Add the new package to the parent class UserPackages array
            this.getParent().getUserPackages().push({ namespace   : this.getPkgNamespace(),
                                                      status      : "disabled",
                                                      version     : this.getPkgVersion(),
                                                      displayName : this.getPkgName()
                                                    });

            // Load and instantiate the new module
            this.getParent()._loadPackage( this.getPkgNamespace() );
