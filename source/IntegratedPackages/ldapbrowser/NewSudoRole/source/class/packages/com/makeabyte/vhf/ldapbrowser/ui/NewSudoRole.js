...
qx.Class.define( "packages.com.makeabyte.vhf.ldapbrowser.ui.NewSudoRole", {
  ...
  construct : function( obj ) {
      ...
      this.setCallback( obj[1] );
           ...
           this.setNewSudoWin( new qx.ui.window.Window( "New Sudo Role", "icon/16/" + this.getWireFrame().getAppIcon() ) );
           this.getNewSudoWin().set({
		          showMinimize: true,
		          allowMinimize: false,
		          allowMaximize: true
           });
    	   this.getNewSudoWin().setSpace( 200, 400, 0, 450 );  // left spacing // width // top spacing // height
    	   this.getNewSudoWin().removeAll();

           // Window icon
           var ico1 = new qx.ui.basic.Atom( "New Sudo Role", "icon/32/categories/applications.png")
		   with( ico1 ) {
         ...
         */
         ...
         var btnCancel = new qx.ui.form.Button( "Cancel", "icon/16/actions/dialog-cancel.png" );
             btnCancel.addEventListener( "execute", function( e ) { this.getNewSudoWin().close(); }, this );
             btnCancel.set({ bottom : 10, right : 65 });

         var btnOK = new qx.ui.form.Button( "OK", "icon/16/actions/dialog-ok.png" );
             btnOK.addEventListener( "execute", function( e ) {

                    // Assemble the sudo role object
                    var dn = "cn=" +  txtName.getValue() + "," + this.getParentOuDn();
                   ...
                   for( i=0; i<count; i++ )
                   	    // Strip the cn from the dn as this attribute should always be the same as the uid
                   ...
                   var callback = qx.lang.Function.bind( this.__addRoleHandler, this );
                   this.getWireFrame()._callAsyncRpc( callback, "core.rpc.API", "invoke", "SUDO", "addRole", dn,
                                                                                                             txtName.getValue(),
        ...
        this.getNewSudoWin().add( btnOK, btnCancel );
        this.getNewSudoWin().add( ico1, tv1 );
	    this.getWireFrame()._addToWorkspace( this.getNewSudoWin() );

	    // Display copyright
        var lblCopyright = new qx.ui.basic.Label( this.getWireFrame().getCopyright() );
            lblCopyright.set({ bottom: 15, left: 10 });
        this.getNewSudoWin().add( lblCopyright );
		this.getNewSudoWin().open();
  ...
  */
  ...
  members : {
    ...
    */
    __addRoleHandler : function( result, ex, id ) {
